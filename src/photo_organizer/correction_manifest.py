...
def _rule_from_mapping(mapping: dict[str, Any], default_priority: CorrectionPriority) -> CorrectionRule:
    selector_type = _string_or_none(mapping.get("selector_type"))
    selector = None
    for candidate_type in ("file", "path", "folder", "glob", "pattern"):
    ...
    if selector_type is None:
        selector_type = "glob"
    if selector is None:
        raise CorrectionManifestError("Correction rule requires file, folder, glob or selector")
...
def _matches_rule(rule: CorrectionRule, file_path: Path, source_root: Path | None) -> bool:
    ...
    if selector_type == "folder":
        ...
        return any(parent.name == folder for parent in file_path.parents) or rel_path.startswith(f"{folder}/")
...
def correction_for_file(
    ...
    matches = [rule for rule in manifest.rules if _matches_rule(rule, path, root)]
    if not matches:
        return None
